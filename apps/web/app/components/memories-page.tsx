  ...
  return (
    <section className="page-panel memories-page">
      <div className="page-hero memory-hero">
        <div><span className="page-context">PostgreSQL · review candidates</span><h1>长期记忆</h1><p>当前只生成待审候选，不会自动激活或召回。每条候选都必须关联产生它的思绪运行、来源说话人和证据消息。</p></div>
        <div className="dual-stat"><span><strong>{memories.length}</strong>候选</span><i /><span><strong>{unresolvedCount}</strong>归因待定</span></div>
      </div>

      <div className="memory-boundary-note"><LuShieldCheck aria-hidden /><strong>当前边界</strong><span>候选生成已接入真实模型；正式记忆库、激活审核与召回排序尚未实现。</span></div>
      {error && <div className="inline-error">{error} <button onClick={() => void load()}>重试</button></div>}

      <div className="memory-toolbar">
        <label className="search-field"><LuSearch aria-hidden /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索候选、说话人或会话" /></label>
      ...
      {loading ? (
        ...
        <div className="empty-state"><LuDatabase className="empty-icon" aria-hidden /><h3>没有匹配的真实记忆候选</h3><p>memory_consolidation 处理到新的 QQ 消息并找到可靠事实后，候选会出现在这里。</p></div>
      ) : (
        <div className="memory-candidate-list">
          {filtered.map((memory) => (
            <article key={memory.id}>
              <header>
                <span className="memory-type"><LuDatabase aria-hidden />{memory.operation}</span>
              ...
              <footer>
                ...
                <button className="ghost-button" onClick={() => onOpenThought(memory.thought_run_id)}><LuExternalLink aria-hidden />查看来源思绪</button>
              </footer>
            </article>
          ))}
        </div>
