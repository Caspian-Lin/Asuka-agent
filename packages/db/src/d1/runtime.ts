...
export async function ensureD1Schema(database: AnyD1Database) {
  let ready = schemaReady.get(database as object);
  if (!ready) {
    const initialization = database.batch(
      schemaStatements.map((statement) => database.prepare(statement)),
    ).then(() => undefined);
    schemaReady.set(database as object, initialization);
    ready = initialization;
  }
  await ready;
  return createD1Db(database);
}

export function resetSchemaReadyForTests(database: AnyD1Database) {
