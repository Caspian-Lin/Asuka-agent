...
设置页提供两套纯配色主题：`classic` 保留明亮黑白基础主题；`asuka` 使用深红导航面、金橙工作区和浅亮蓝交互强调。主题只覆盖语义颜色 token，不改变字体、字号、间距、组件尺寸或信息结构。选择保存在浏览器 `localStorage`，不进入 PostgreSQL，也不影响 Agent 或模型运行配置；页面在 React hydration 前恢复主题以减少首屏颜色闪烁。
